...
-   In this data set, the variable `listings` is the number of total active listings.
:::
...
head(dallas_sub)
```
- The reason the code given in Problem 4 is incorrect is because of this use of `&` in the code block. This causes the code to group `Dallas` and `2012` to display all 2012 columns for Dallas, then displaying `2013` columns for every city. Replacing `&` with `,` fixes this issue.
:::
